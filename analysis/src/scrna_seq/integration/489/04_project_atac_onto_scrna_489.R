#!/usr/bin/env Rscript
# 04_project_atac_onto_scrna_489.R
...
#
# Usage: Rscript 04_project_atac_onto_scrna_489.R [full|balanced|both]
# Prerequisites: 00_prepare_scrna_489.R and 01_label_transfer_489.R must have run.
...
for (MODE in modes) {
  ...
  write.csv(score_df,
    file.path(OUT_DIR, "tables", paste0("mapquery_score_by_celltype_489_", MODE, ".csv")),
    row.names = FALSE)
  ...
  p_scrna <- DimPlot(seRNA, reduction = "umap", group.by = "cell_type",
             ...
             ggtitle(paste0("489 scRNA (", scrna_tag, ")\n", ncol(seRNA), " cells")) +
  ...
  p_atac_proj <- DimPlot(atac_mapped, reduction = "ref.umap",
                          ...
                          pt.size = 0.3, label = TRUE, repel = TRUE) +
                 ggtitle(paste0("489 ATAC projected onto scRNA UMAP\n",
  ...
  p_score <- ggplot(atac_coords, aes(x=UMAP_1, y=UMAP_2, color=score)) +
    ...
    ggtitle(paste0("489 ATAC: prediction confidence on scRNA UMAP (", MODE, ")")) +
    theme_bw() + theme(axis.title=element_blank(), axis.text=element_blank(),
                       axis.ticks=element_blank(), panel.grid=element_blank())

  # Save panels
  pdf(file.path(PLOT_DIR, paste0("scrna_umap_489_", MODE, ".pdf")), width=8, height=6)
  print(p_scrna); dev.off()

  pdf(file.path(PLOT_DIR, paste0("atac_projected_489_", MODE, ".pdf")), width=8, height=6)
  print(p_atac_proj); dev.off()

  pdf(file.path(PLOT_DIR, paste0("overlay_atac_on_scrna_489_", MODE, ".pdf")), width=8, height=6)
  print(p_overlay); dev.off()

  pdf(file.path(PLOT_DIR, paste0("projection_score_map_489_", MODE, ".pdf")), width=8, height=6)
  print(p_score); dev.off()

  # Combined 4-panel summary
  pdf(file.path(PLOT_DIR, paste0("summary_4panel_489_", MODE, ".pdf")), width=18, height=12)
