...
data.path   <- "/projectnb/paxlab/presh/projects/spatial_atac/Data/alleloscope/lowseq_tissue_from_existing/combined_488B_489"
...
Obj <- Createobj(
  ...
  ref_all          = ref_all,
  var_all          = var_all,
  samplename       = "lowseq_488B_489_combined",
...
Obj_filtered <- tryCatch(
  ...
  error = function(e) {
    message(sprintf("  [warn] Genotype_value failed (%s); skipping to Step 9 without gtv", e$message))
    Obj_filtered
  }
)
...
Obj_filtered <- tryCatch(
  ...
  error = function(e) {
    message(sprintf("  [warn] Genotype failed (%s); saving Obj as-is", e$message))
    Obj_filtered
  }
)
...
if (sum(keep_cells) == 0L) {
  message("  [warn] No cells have coverage data in chr1000k_fragments.tsv; skipping CNV heatmap")
  cov_obj <- NULL
} else {
