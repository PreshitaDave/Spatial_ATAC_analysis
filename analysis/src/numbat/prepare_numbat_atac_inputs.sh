...
FRAG_FILE_DEFAULT="${PROJECT_ROOT}/Data/${DATASET}.fragments.sort.filtered.bed"
if [[ "${DATASET}" == "deepseq" ]]; then
  FRAG_FILE_DEFAULT="${PROJECT_ROOT}/Data/deepseq.fragments.sort.filtered.bed.gz"
fi

FRAG_FILE="${FRAG_FILE:-${FRAG_FILE_DEFAULT}}"
BARCODE_FILE_ATAC="${NUMBAT_INPUT_DIR}/${DATASET}_atac_barcodes.tsv"
BARCODE_FILE_PILEUP="${NUMBAT_INPUT_DIR}/${DATASET}_atac_barcodes_for_pileup.tsv"
AGG_ANNOT_FILE="${NUMBAT_INPUT_DIR}/${DATASET}_atac_barcodes_with_group.tsv"
BIN_GR="${NUMBAT_REF_DIR}/var220kb.rds"
ATAC_BIN_RDS="${NUMBAT_INPUT_DIR}/${DATASET}_atac_bin.rds"
...
MERGED_BAM="${NUMBAT_INPUT_DIR}/${DATASET}_merged_for_numbat.bam"
...
TARGETED_BAM_PATTERN="${PROJECT_ROOT}/Data/variant_calling/${DATASET}/Bam/%s.filter.targeted.bam"
FULL_BAM_PATTERN="${PROJECT_ROOT}/Data/variant_calling/${DATASET}/Bam/${DATASET}_%s.filter.bam"

# Tissue-specific barcode file from Alleloscope barcode-matching pipeline.
# These are 16bp barcodes WITHOUT -1; we append -1 to match the fragments file.
# TISSUE defaults to 488B; set TISSUE=489 to run on the other tissue.
TISSUE="${TISSUE:-488B}"
TISSUE_BARCODE_FILE="${PROJECT_ROOT}/Data/alleloscope/barcodes/${DATASET}_${TISSUE}.barcodes.tsv"
...
# Build ATAC barcode file for fragment matrix: append -1 to match fragment
# barcode format.
...
if [[ ! -f "${ALLELE_DF}" ]]; then
  ...
  pileup_vcf="${ALLELE_OUTDIR}/pileup/${DATASET}_atac/cellSNP.base.vcf"
