#!/usr/bin/env python3
"""
Run1-specific adaptation of ../export_for_R.py (not editing the original, which is
co-developed with the separate rigid-transform/run2 effort). Same logic, pointed at
run1_workspace/mosaicfield_outputs/ so nothing here ever touches the shared
analysis/src/alignment/mosaicfield_outputs/ directory. Run from run1_workspace/.
"""
...
print("\n--- Nonlinear alignment (reusing mosaic_run1_isolated.ipynb STEP 4b output) ---")
_voronoi_csv = os.path.join(outdir, "xenium_to_atac_mapping_voronoi.csv")
if not os.path.exists(_voronoi_csv):
    raise FileNotFoundError(
        f"{_voronoi_csv} not found -- run mosaic_run1_isolated.ipynb's STEP 4b first "
        f"(this script does not recompute the nonlinear-stage mapping itself)."
    )
...
print(f"\nrun1_workspace/mosaicfield_outputs/r_comparison/ -- isolated from shared mosaicfield_outputs/r_comparison/")
