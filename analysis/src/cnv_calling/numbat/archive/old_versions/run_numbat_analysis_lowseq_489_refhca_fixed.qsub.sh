#!/bin/bash
#
# run_numbat_analysis_lowseq_489_refhca_fixed.qsub.sh
# Purpose: NUMBAT refhca analysis for lowseq_489 with CORRECTED barcode matching
# Prerequisites: Regenerated ATAC matrix using correct pileup barcodes
#

#$ -N numbat_refhca_489_fixed
#$ -l h_rt=08:00:00
#$ -pe omp 8
#$ -P paxlab
#$ -l mem_per_core=8G
#$ -o analysis/qsub_logs/numbat_refhca_489_fixed_$JOB_ID.log
...
echo "════════════════════════════════════════════════════════════════════════════"
echo "NUMBAT refhca Analysis: lowseq_489 (with corrected barcodes)"
echo "════════════════════════════════════════════════════════════════════════════"
