#!/usr/bin/env Rscript
# =============================================================================
# Full Alleloscope analysis for deepseq scATAC-seq (no matched DNA)
...
data.path   <- "/projectnb/paxlab/presh/projects/spatial_atac/Data/alleloscope/deepseq"
dir_path    <- file.path(data.path, "output")
dir.create(dir_path, showWarnings = FALSE, recursive = TRUE)

message(sprintf("[%s] === Alleloscope deepseq analysis START (ncores=%d) ===",
                Sys.time(), NCORES))
...
# cont=TRUE reuses existing chromosome-level EM outputs if present.
# ─────────────────────────────────────────────────────────────────────────────
if (RESUME_FROM <= 3L) {
...
Obj_filtered <- Segmentation(
  Obj_filtered = Obj_filtered,
  raw_counts   = raw_counts,
  ref_counts   = ref_counts_bulk,
  plot_seg     = TRUE
)

n_segs <- nrow(Obj_filtered$seg_table_filtered)
