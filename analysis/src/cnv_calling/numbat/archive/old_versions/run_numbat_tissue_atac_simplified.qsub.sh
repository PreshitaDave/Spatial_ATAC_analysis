#!/bin/bash
################################################################################
# run_numbat_tissue_atac_simplified.qsub.sh
#
# Purpose: Run NUMBAT on tissue-specific ATAC bin data with monopogen variants
#
# Input:
#   - Tissue-specific bin matrices: lowseq_488B_atac_bin.rds, lowseq_489_atac_bin.rds
#   - SNP VCF: genome1K.phase3.SNP_AF5e2.chr1toX.hg38.vcf
#   - Monopogen VCF files for allele information
#
# Output:
...
#$ -P paxlab
#$ -N numbat_tissue_atac
#$ -wd /projectnb/paxlab/presh/projects/spatial_atac
#$ -pe omp 8
#$ -l mem_per_core=8G
#$ -l h_rt=24:00:00
#$ -o /projectnb/paxlab/presh/projects/spatial_atac/analysis/qsub_logs/numbat_tissue_atac.$JOB_ID.out
#$ -e /projectnb/paxlab/presh/projects/spatial_atac/analysis/qsub_logs/numbat_tissue_atac.$JOB_ID.err
#$ -j n
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] ===== NUMBAT TISSUE-SPECIFIC ATAC ANALYSIS ====="
...
for TISSUE in 488B 489; do
  BIN_RDS="Data/04_analysis/cnv/numbat/inputs/lowseq_${TISSUE}_atac_bin.rds"
  if [[ ! -f "$BIN_RDS" ]]; then
...
if (!is.na(allele_file) && file.exists(allele_file)) {
  ...
  tryCatch({
    ...
    # Basic copy number calling based on bins
    # (Full NUMBAT would use SNP phase info, but we'll use basic bin-based inference)
    
    # For now, just save the data we have
