#!/usr/bin/env Rscript
# 00_prepare_scrna_488B.R
# Finalize scRNA-seq QC for tissue 488B (SQ_DR_488) and save processed Seurat object.
# Parameters follow tiss_488B_explore.Rmd: nFeature_RNA>200, percent.mt<20, dims=1:15.
# Outputs:
#   seurat_scrna_488B_processed.rds  — full object (all cells)
#   seurat_scrna_488B_balanced.rds   — downsampled to equal cells per cell type
...
# ── Paths ─────────────────────────────────────────────────────────────────────
DATA_10X <- "/projectnb/paxlab/DATA/DriesSpatial/scRNAseq/SQ_DR_488/outs/filtered_feature_bc_matrix"
...
obj    <- CreateSeuratObject(counts, project = "tiss_488B", min.cells = 3, min.features = 200)
...
p_elbow <- ElbowPlot(obj, ndims = 30) + ggtitle("488B scRNA — PCA elbow plot")
...
p_umap <- DimPlot(obj, reduction = "umap", label = TRUE, repel = TRUE) +
          ggtitle("488B scRNA-seq clusters")
...
write.csv(markers, file.path(TAB_DIR, "scrna_488B_cluster_markers.csv"), row.names = FALSE)
...
p_dot <- DotPlot(obj, features = lapply(canonical, head, 3),
         ...
         RotatedAxis() + ggtitle("488B: canonical markers by cluster")
...
p_celltypes <- DimPlot(obj, group.by = "cell_type", label = TRUE, repel = TRUE) +
               ggtitle("488B scRNA-seq — annotated cell types")
ggsave(file.path(PLOT_DIR, "umap_cell_types.pdf"), p_celltypes, width = 8, height = 6)

cat("\nCell type composition (488B — full):\n")
print(table(obj$cell_type))
write.csv(data.frame(table(obj$cell_type)),
          file.path(TAB_DIR, "scrna_488B_celltype_counts.csv"), row.names = FALSE)
write.csv(obj@meta.data, file.path(TAB_DIR, "scrna_488B_metadata.csv"))

# ── Save full object ───────────────────────────────────────────────────────────
out_full <- file.path(OBJ_DIR, "seurat_scrna_488B_processed.rds")
...
p_bal <- DimPlot(obj_balanced, group.by = "cell_type", label = TRUE, repel = TRUE) +
         ggtitle(paste0("488B scRNA — balanced reference (", min_n, " cells/type)"))
ggsave(file.path(PLOT_DIR, "umap_cell_types_balanced.pdf"), p_bal, width = 8, height = 6)

out_bal <- file.path(OBJ_DIR, "seurat_scrna_488B_balanced.rds")
saveRDS(obj_balanced, out_bal)
cat("Saved balanced object:", out_bal, "\n")

write.csv(data.frame(table(obj_balanced$cell_type)),
          file.path(TAB_DIR, "scrna_488B_celltype_counts_balanced.csv"), row.names = FALSE)
...
for (tag in c("processed", "balanced")) {
  ...
  writeMM(nm, file.path(OBJ_DIR, paste0("scrna_488B_norm_matrix_", tag, ".mtx")))
  write.csv(rownames(nm), file.path(OBJ_DIR, paste0("scrna_488B_genes_", tag, ".csv")),    row.names = FALSE, col.names = FALSE)
  write.csv(colnames(nm), file.path(OBJ_DIR, paste0("scrna_488B_barcodes_", tag, ".csv")), row.names = FALSE, col.names = FALSE)
  write.csv(o@meta.data[, c("seurat_clusters", "cell_type", "nFeature_RNA", "nCount_RNA", "percent.mt")],
            file.path(OBJ_DIR, paste0("scrna_488B_cell_metadata_", tag, ".csv")))
}
