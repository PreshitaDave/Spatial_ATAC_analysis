#!/bin/bash
################################################################################
# run_numbat_analysis_lowseq_489_refhca.qsub.sh
#
# PURPOSE: Run NUMBAT CNV analysis for lowseq_489 using built-in ref_hca
...
#
# INPUTS:
#   - ATAC bin matrix: Data/04_analysis/cnv/numbat/inputs/lowseq_489/atac_bin/lowseq_489_atac_bin.rds
#   - Allele counts: Data/04_analysis/cnv/numbat/inputs/lowseq_489/alleles/lowseq_489_atac_allele_counts.tsv.gz
#   - Reference: ref_hca (built into NUMBAT package)
#
# OUTPUTS:
#   - CNV calls, plots, and analysis results in Data/04_analysis/cnv/numbat/results/lowseq_489/
#
################################################################################

#$ -N numbat_analysis_lowseq_489_refhca
#$ -l h_rt=08:00:00
#$ -pe omp 8
#$ -P paxlab
#$ -l mem_per_core=8G
#$ -o analysis/qsub_logs/numbat_analysis_lowseq_489_refhca_$JOB_ID.out
#$ -e analysis/qsub_logs/numbat_analysis_lowseq_489_refhca_$JOB_ID.err
...
if [[ $VALIDATION_EXIT -ne 0 ]]; then
  ...
  echo "[$(date +'%Y-%m-%d %H:%M:%S')] "
  echo "[$(date +'%Y-%m-%d %H:%M:%S')] Run this command:"
...
OUTPUT_DIR="Data/04_analysis/cnv/numbat/results/${TISSUE}/"
