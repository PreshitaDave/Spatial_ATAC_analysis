#!/usr/bin/env Rscript
# 03_coembed_488B.R
# Option 3: Co-embedding for tissue 488B (deepseq_488B).
...
#
# Usage: Rscript 03_coembed_488B.R [full|balanced]
#   full     — uses archr_488B_with_integration + seurat_scrna_488B_processed.rds
#   balanced — uses archr_488B_with_integration_balanced + seurat_scrna_488B_balanced.rds
#
# Prerequisites: 01_label_transfer_488B.R must have been run for the chosen mode.
...
ARCHR_INT <- file.path(OBJ_DIR, paste0("archr_488B_with_integration_", MODE))
SCRNA_RDS <- file.path(OBJ_DIR, paste0("seurat_scrna_488B_", scrna_tag, ".rds"))
...
if (!dir.exists(ARCHR_INT)) stop("Run 01_label_transfer_488B.R (", MODE, ") first: ", ARCHR_INT)
...
p_combined1 <- plotEmbedding(proj, colorBy = "cellColData", name = "predictedGroup_co",
               ...
               ggtitle(paste0("488B joint UMAP — predicted cell type (", MODE, ")"))
p_combined2 <- plotEmbedding(proj, colorBy = "cellColData", name = "Clusters",
                              embedding = "UMAP_Combined", size = 0.5, plotAs = "points") +
               ggtitle("488B joint UMAP — original ATAC clusters")
p_combined3 <- plotEmbedding(proj, colorBy = "cellColData", name = "Clusters_Combined",
                              embedding = "UMAP_Combined", size = 0.5, plotAs = "points") +
               ggtitle("488B joint UMAP — combined clusters")

pdf(file.path(PLOT_DIR, paste0("3a_archr_joint_umap_488B_", MODE, ".pdf")), width = 18, height = 6)
print(p_combined1 | p_combined2 | p_combined3)
dev.off()
cat("Saved: 3a_archr_joint_umap_488B_", MODE, ".pdf\n")
...
atac_seurat <- CreateSeuratObject(counts = gs_mat, assay = "ACTIVITY", project = "atac_488B")
...
p_bridge <- DimPlot(atac_seurat, reduction = "umap_lsi",
            ...
            ggtitle(paste0("488B Seurat bridge: predicted cell type (", MODE, " ref)")) +
            theme(legend.position = "right")
ggsave(file.path(PLOT_DIR, paste0("3b_seurat_bridge_umap_488B_", MODE, ".pdf")),
...
write.csv(comparison,
          file.path(TAB_DIR, paste0("coembed_method_comparison_488B_", MODE, ".csv")),
          row.names = FALSE)
...
p_agree <- ggplot(as.data.frame(agree_table), aes(x=Var2, y=Var1, fill=Freq)) +
  ...
  labs(x="Seurat prediction", y="ArchR prediction",
       title=paste0("488B: ArchR vs Seurat agreement (", MODE, " ref)"),
       fill="N cells") +
  theme_bw() + theme(axis.text.x = element_text(angle=45, hjust=1))
ggsave(file.path(PLOT_DIR, paste0("3b_archr_vs_seurat_agreement_488B_", MODE, ".pdf")),
       p_agree, width=9, height=8)

# ── Save ──────────────────────────────────────────────────────────────────────
saveRDS(atac_seurat, file.path(OBJ_DIR, paste0("seurat_atac_bridge_488B_", MODE, ".rds")))
...
cat("\nOption 3 co-embedding (", MODE, ") complete for 488B.\n")
