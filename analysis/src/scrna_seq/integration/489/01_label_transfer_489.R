#!/usr/bin/env Rscript
# 01_label_transfer_489.R
# CCA-based label transfer from scRNA → ATAC (tissue 489 / lowseq).
#
# Usage: Rscript 01_label_transfer_489.R [full|balanced]
#   full     — use seurat_scrna_489_processed.rds  (default, all cells)
#   balanced — use seurat_scrna_489_balanced.rds   (equal cells per cell type)
#
# Prerequisites: 00_prepare_scrna_489.R must have been run.
...
# ── Paths ─────────────────────────────────────────────────────────────────────
ARCHR_DIR <- "/projectnb/paxlab/presh/projects/spatial_atac/Data/01_outputs/archR_objects/5000bp_non_binarize/lowseq_489"
OBJ_BASE  <- "/projectnb/paxlab/presh/projects/spatial_atac/Data/04_analysis/multiomic/scrna_integration/489/objects"
...
SCRNA_RDS <- file.path(OBJ_BASE, paste0("seurat_scrna_489_", scrna_tag, ".rds"))
ARCHR_OUT <- file.path(OBJ_BASE, paste0("archr_489_with_integration_", MODE))
...
if (!file.exists(SCRNA_RDS)) stop("Run 00_prepare_scrna_489.R first: ", SCRNA_RDS)
...
atac_obj <- CreateSeuratObject(counts = gs_mat, assay = "ACTIVITY", project = "atac_489")
...
write.csv(score_by_type,
          file.path(TAB_DIR, paste0("prediction_score_by_celltype_489_", MODE, ".csv")),
          row.names = FALSE)
...
p1 <- plotEmbedding(proj, colorBy = "cellColData", name = "predictedGroup_co",
      ...
      ggtitle(paste0("489 ATAC — predicted cell type (", MODE, " ref)"))
p2 <- plotEmbedding(proj, colorBy = "cellColData", name = "predictedScore_co",
                    embedding = "UMAP_LSI", size = 0.5, plotAs = "points") +
      ggtitle("489 ATAC — prediction confidence") + scale_color_viridis_c()
p3 <- plotEmbedding(proj, colorBy = "cellColData", name = "Clusters",
                    embedding = "UMAP_LSI", size = 0.5, plotAs = "points") +
      ggtitle("489 ATAC — original clusters")

pdf(file.path(PLOT_DIR, paste0("umap_label_transfer_489_", MODE, ".pdf")), width = 18, height = 6)
print(p1 | p2 | p3)
dev.off()
...
pdf(file.path(PLOT_DIR, paste0("confusion_heatmap_489_", MODE, ".pdf")), width = 10, height = 8)
...
p_score <- ggplot(meta, aes(x = predictedScore_co, fill = predictedGroup_co)) +
  ...
  labs(x = "Prediction score", y = "N cells",
       title = paste0("489: label transfer confidence by cell type (", MODE, " ref)"),
       caption = "Red=0.5, Blue=0.7 thresholds") +
  theme_bw() + theme(legend.position = "none")
ggsave(file.path(PLOT_DIR, paste0("prediction_score_distribution_489_", MODE, ".pdf")),
...
write.csv(meta_full, file.path(TAB_DIR, paste0("label_transfer_results_489_", MODE, ".csv")))

writeMM(imputed_mat, file.path(OBJ_BASE, paste0("archr_489_GeneIntegrationMatrix_", MODE, ".mtx")))
write.csv(data.frame(gene = rownames(imputed_mat)),
          file.path(OBJ_BASE, paste0("archr_489_gene_names_", MODE, ".csv")), row.names = FALSE)
write.csv(data.frame(cell = colnames(imputed_mat)),
          file.path(OBJ_BASE, paste0("archr_489_cell_names_", MODE, ".csv")), row.names = FALSE)

cat("\nLabel transfer (", MODE, ") complete for 489.\n")
